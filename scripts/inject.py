...
from _common import CSS_TAG, MDWRAP_MARKER, ensure_feedback_dir, find_files
...
def inject_one(path: Path) -> str:
    ...
    if MDWRAP_MARKER in text:
        return "skipped (generated by mdwrap.py — use mdwrap.py to manage this file)"
    changed = False
    notes = []
...
def main() -> int:
    ...
    htmls = find_files(root, "*.html", args.recursive)
    if not htmls:
