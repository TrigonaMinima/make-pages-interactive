...
(function () {
  ...
  // Compact per-element summary shared by every place that lists selected
  // elements: the element-selection editor, the edit-existing-comment flow,
  // and the pending-list row. `maxLen`/`showCfId` let each caller keep its
  // existing look (the editor shows more text plus the internal cf-id; the
  // compact pending row does not) without duplicating the template.
  function elementQuoteHtml(elements, opts) {
    const { maxLen = 80, showCfId = true } = opts || {};
    return elements.map(e => `<div>${escapeHtml(e.tag)}${e.id ? "#" + escapeHtml(e.id) : ""}${showCfId && e.cf_id ? " <span style='opacity:0.5'>(" + e.cf_id + ")</span>" : ""} — <span style="opacity:0.7">${escapeHtml(e.text_snippet.slice(0, maxLen))}${e.text_snippet.length > maxLen ? "…" : ""}</span></div>`).join("");
  }

  // Renders the editor's quote/header area for any comment type — shared by
  // every open* function below (both "create" and "edit existing") so a new
  // comment type only needs to be taught to this one place.
  function renderEditorQuote(type, data) {
    data = data || {};
    const quoteEl = $("cf-editor-quote");
    quoteEl.classList.toggle("cf-comment-general", type === "general");
    if (type === "general") {
      quoteEl.textContent = "General question";
    } else if (type === "elements") {
      quoteEl.innerHTML = elementQuoteHtml(data.elements || []);
    } else {
      quoteEl.textContent = '"' + (data.quote || "") + '"';
    }
  }

  // The editor is position: fixed → viewport coords, NO scrollY. Used by any
  // open path that has no source element/selection to anchor a popup to.
  ...
  function openTextCommentEditor() {
    ...
    const editor = $("cf-editor");
    renderEditorQuote("selection", { quote: savedTextSelection.quote });
    editor._payload = {
      type: "selection",
      comment: "",
    ...
    $("cf-editor-save").textContent = "add (⌘↵)";
  ...
  function openElementCommentEditor() {
    ...
    renderEditorQuote("elements", { elements });
    editor._payload = {
      type: "elements",
      comment: "",
      elements,
  ...
  // Reopen the editor against an already-pending comment. Its quote/anchor is
  // rendered straight from the stored comment (same renderEditorQuote() every
  // other open path uses) so the selection this comment is anchored to is
  // never touched — only `comment` text is editable. `_payload` carries just
  // `id` + `editing: true`; saveEditorComment() looks the rest up from
  // `pending` by id rather than round-tripping the whole comment through the
  // editor.
  function openEditExisting(c) {
    const editor = $("cf-editor");
    renderEditorQuote(c.type, c);
    editor._payload = { id: c.id, editing: true };
    $("cf-editor-text").value = c.comment || "";
    $("cf-editor-save").textContent = "save (⌘↵)";
    centerEditor();
    editor.classList.add("cf-visible");
    setTimeout(() => {
      const t = $("cf-editor-text");
      t.focus();
      t.select();
    }, 50);
  }
  ...
  function saveEditorComment() {
    ...
    if (payload.editing) {
      // Update in place — selection/anchor/type are untouched, only the
      // comment text changes.
      const idx = pending.findIndex((x) => x.id === payload.id);
      if (idx !== -1) pending[idx] = Object.assign({}, pending[idx], { comment: text });
    } else {
      payload.id = "c-" + Date.now() + "-" + Math.random().toString(36).slice(2, 6);
      payload.comment = text;
      payload.created_at = new Date().toISOString();
      pending.push(payload);
      // Exit element mode / clear selection after committing a new comment.
      // Nothing "live" to tear down when editing an already-pending one.
    ...
    showToast(payload.editing ? "comment updated" : "comment added");
  }
  ...
  function renderPending() {
    ...
    pending.forEach((c) => {
      ...
      if (c.type === "general") {
        ...
        quote.innerHTML = elementQuoteHtml(c.elements, { maxLen: 60, showCfId: false });
      } else {
      ...
      edit.className = "cf-comment-edit";
      edit.textContent = "edit";
      edit.addEventListener("click", () => openEditExisting(c));
